...
const MyJobs = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`http://localhost:5000/myJobs/xyz@gmail.com`)
      .then((res) => res.json())
      .then((data) => {
        setJobs(data);
        setFilteredJobs(data); // Set filtered jobs to the full job list initially
        setIsLoading(false);
      })
      .catch((error) => {
    ...
    fetch(`http://localhost:5000/job/${id}`, {
      ...
      .then((data) => {
        if (data.acknowledged === true) {
          ...
          // Remove the deleted job from the filtered list
          setFilteredJobs((prevJobs) => prevJobs.filter((job) => job._id !== id));
          // Also update the original jobs list to ensure it reflects the deletion
